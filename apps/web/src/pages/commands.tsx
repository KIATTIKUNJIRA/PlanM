...
export default function CommandsChecklistPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 flex flex-col lg:flex-row gap-6">
      <aside className="w-full lg:w-64 lg:sticky top-4 h-fit bg-white border rounded-lg p-4 text-sm space-y-3">
        <h2 className="font-semibold text-gray-800">Command Catalog</h2>
        <div className="space-y-2">
          ...
          />
          <select value={showStatus} onChange={e => setShowStatus(e.target.value as any)} className="w-full border rounded px-2 py-1 text-sm">
            <option value="all">ทุกสถานะ</option>
            <option value="todo">TODO</option>
            <option value="wip">WIP</option>
            <option value="done">DONE</option>
          </select>
          <select value={showRoleRestricted} onChange={e => setShowRoleRestricted(e.target.value as any)} className="w-full border rounded px-2 py-1 text-sm">
      ...
      {COMMAND_CATALOG.map(sec => (
        ...
        <a href={`#section-${sec.id}`} className="block font-medium text-gray-700 hover:text-black">{sec.label}</a>
              <div className="flex flex-wrap gap-1">
                {(sec.children || []).slice(0,6).map(ch => {
                  const st = (statuses[ch.id] || ch.status || 'todo') as Status;
                  return <span key={ch.id} className={`px-1.5 py-0.5 rounded text-[10px] ${statusColor(st)}`}>{ch.label}</span>;
                })}
              </div>
            </div>
          ))}
        ...
        <div className="pt-4 space-y-2 border-t mt-4">
          <button onClick={() => markAll(flat.map(f => f.id), 'done')} className="w-full text-xs px-2 py-1 rounded bg-indigo-600 text-white hover:bg-indigo-500">Mark All Done</button>
          <button onClick={() => clearAll()} className="w-full text-xs px-2 py-1 rounded bg-gray-200 text-gray-700 hover:bg-gray-300">Clear All</button>
        </div>
      </aside>
      <main className="flex-1 space-y-10">
        {Object.keys(grouped).length === 0 && (
          <p className="text-sm text-gray-500">ไม่พบรายการที่ตรงเงื่อนไข</p>
        )}
          ...
          return (
            <section key={secId} id={`section-${secId}`} className="space-y-4 scroll-mt-24">
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-semibold">{secMeta?.label || secId}</h3>
                {secMeta?.importance === 'primary' && <span className="text-xs px-2 py-0.5 rounded bg-indigo-100 text-indigo-700">Primary</span>}
              </div>
                  ...
                  return (
                    <div key={it.id} className="p-3 flex flex-col sm:flex-row sm:items-center gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="font-medium text-sm truncate" title={it.label}>{it.label}</span>
                          {restricted && <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-200 text-slate-600">{it.roles?.join('/')}</span>}
                        </div>
                        {it.desc && <p className="text-xs text-gray-500 mt-0.5">{it.desc}</p>}
                        {it.route && <a className="text-xs text-blue-600 hover:underline" href={it.route}>{it.route}</a>}
                      </div>
                      <div className="flex items-center gap-2 self-start sm:self-center">
                        <button
                          onClick={() => toggleStatus(it.id)}
                          className={`text-xs font-medium px-2 py-1 rounded ${statusColor(st)} hover:opacity-90`}
                        ...
                        <button
                          ...
                          className={`inline-flex items-center gap-1 rounded-md px-2 py-1 border text-[11px] font-medium transition bg-white hover:bg-gray-50 disabled:hover:bg-white ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
